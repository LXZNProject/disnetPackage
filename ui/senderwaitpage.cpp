...
senderwaitpage::senderwaitpage(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::senderwaitpage)
{
    ui->setupUi(this);
...
senderwaitpage::~senderwaitpage()
{
    delete ui;
}

void senderwaitpage::initSenderWaitPage()
{
...
void senderwaitpage::sendNote(SenderDesc_t savePackInfo)
{
    //调用短信发送接口
    NoteDesc_t noteInfo;
    QString smsContent;
    ...
    noteInfo.mSmsContent = smsContent.arg(savePackInfo.mobilephone).arg(savePackInfo.companyName).arg(\
...
void senderwaitpage::readWaitInfo(QByteArray param)
{
    waitSaveTime = 120;
    QByteArray data;
...
void senderwaitpage::queryLockStatus()
{
    qint8 status = -1;
...
void senderwaitpage::commitExceptionInfo(QString notClose)
{
